...
local function pickWeightedBudget(pool, targetPerPick)
    local total = 0
    for _, e in ipairs(pool) do
        e._dynWeight = e.catWeight * budgetWeight(e.item.value, targetPerPick)
...
-- 从候选列表中按品类权重随机选取（不考虑预算，用于填充阶段）
-- @param pool table 候选 entry 列表
local function pickWeightedCategory(pool)
    local total = 0
    for _, e in ipairs(pool) do
        total = total + e.catWeight
    end
    if total <= 0 then return pool[1] end
    local r = math.random() * total
    local acc = 0
...
local function pickFromPool(whTypeId, whType, usedNames, targetPerPick, maxValue)
        ...
        if #group > 0 then
            ...
            if #available > 0 then
                if targetPerPick then
                    return pickWeightedBudget(available, targetPerPick)
                else
            ...
            -- 该尺寸组全部用完，允许重名
            if targetPerPick then
                return pickWeightedBudget(group, targetPerPick)
            else
    ...
    -- fallback: 从全池选取
    if targetPerPick then
        return pickWeightedBudget(pool.allItems, targetPerPick)
    else
...
local function sampleSessionParams(whType, whTypeId)
    local baseValue = whType.expectedValue  -- 10万

    -- 1. 采样格子占比：中位数 50%，标准差 ~12%
    --    使正态分布 90% 落在 [30%, 70%] → 1.645σ = 0.20 → σ ≈ 0.12
    local fillRate = 0.50 + randNormal() * 0.12
    fillRate = math.max(0.15, math.min(0.85, fillRate))  -- 硬限制 [15%, 85%]
    ...
    -- 3. 采样目标价值：期望 = baseValue，90% 在 ±50%
    --    正态分布 90% 在 ±1.645σ → 1.645σ = 0.50 * baseValue → σ ≈ 0.304 * baseValue
    local sigma = baseValue * 0.304
    local targetValue = baseValue + randNormal() * sigma
    -- 裁剪到可达范围
    targetValue = math.max(boundMin, math.min(boundMax, targetValue))
    -- 最低不低于 baseValue 的 10%
    targetValue = math.max(baseValue * 0.10, targetValue)

    return targetValue, targetCells
end
...
function WG.Generate(regionId, warehouseTypeId, diffIdx)
    ...
    -- 5. 采样本局参数（目标价值 + 格子占比）
    local targetValue, targetCells = sampleSessionParams(mergedWhType, whTypeId)
    ...
    -- 阶段1：填充物（~75% 格子，纯品类权重，不考虑预算）
    -- 单品价值上限：不超过目标总价的 25%，防止低价场出现超高价物品
    local fillerMaxValue = targetValue * 0.25
    local fillCells = math.floor(targetCells * 0.75)
    local failCount = 0
    ...
    while totalSelectedCells < targetCells and failCount < 10 do
        ...
        local entry = pickFromPool(whTypeId, whType, usedNames, targetPerPick)
        local itemCells = entry.w * entry.h
    ...
    print("[WarehouseGenerator] Generated: " .. warehouseName)
    ...
    print("  Deviation: " .. (totalValue > 0 and (math.floor((totalValue/difficulty.expectedValue - 1) * 100) .. "% from expected") or "N/A"))
