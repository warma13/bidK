...
function GameState.Init(playerCharIdx, regionId, diffIdx, warehouseTypeId, playersConfig)
    ...
    if playersConfig then
        ...
        local playerMoney = state.startingMoney
        if playerChar.bonusMoney then
            playerMoney = math.floor(playerMoney * playerChar.bonusMoney)
        end
...
function GameState.PerformJudgment()
    ...
    if passed then
        ...
        GameState.SecureAddMoney(highBidder, -actualPay)
...
function GameState.EndTiebreak()
    ...
    if state.currentBidder > 0 then
        ...
        GameState.SecureAddMoney(state.currentBidder, -state.currentBid)
        ...
        GameState.SecureAddMoney(idx, -bid)
...
function GameState.SettleWarehouseValue()
    ...
    if profit < 0 then
        ...
        if bonus > 0 then
            for idx = 1, #state.players do
                if idx ~= winner then
                    GameState.SecureAddMoney(idx, bonus)
...
GameState.SecureSetMoney = function(playerIdx, newValue) MoneyManager.SecureSetMoney(playerIdx, newValue) end

--- 安全增减资金
GameState.SecureAddMoney = function(playerIdx, delta) MoneyManager.SecureAddMoney(playerIdx, delta) end
