...
local warehouseModules = {
    ...
    datacenter = "Config.Warehouses.DataCenter",
}
...
local function buildTables(whTypeId)
    ...
    local moduleName = warehouseModules[whTypeId]
    if not moduleName then
        -- fallback 到 grocery
        moduleName = warehouseModules["grocery"]
        whTypeId = "grocery"
        if tableCache[whTypeId] then return tableCache[whTypeId] end
    end
    ...
    local categoryWeights = mod.categoryWeights or {}
...
local function getExpectedValue(tables, level, item)
    ...
    if level >= 2 then
        ...
        -- 无品类信息，回退到全池品质加权均价
        return avg.qualityAvgWeighted[q] or tables.poolAvg
    end
    ...
    if level >= 1 then
        ...
        if cat then
            ...
            -- 该品类无匹配尺寸，回退到品类均价
            return avg.categoryAvg[cat] or tables.poolAvg
        end
...
--- 每件物品根据信息层级取条件期望值，未知物品取池均价
--- @param infoState table 信息状态 { publicInfos, skillInfos, itemRevealLevels }
--- @param whTypeId string|nil 仓库类型ID（nil=使用 grocery）
--- @return number expectedTotal 期望总价值
--- @return number poolAvg 池均价（单件期望）
...
function EstimateValue.CalculateExpected(infoState, whTypeId)
    ...
    if not infoState then
        -- 无信息状态，全部按池均价
        return tables.poolAvg * #items, tables.poolAvg, #items
    end
    ...
    return expectedTotal, tables.poolAvg, #items
end
