...
local DAILY_TASKS = {
    ...
    -- ── 原有任务保留 ─────────────────────────────────────────────────────────
    {
        ...
        title = "拍宝入库",
        desc  = "今日拍得 3 件物品",
        reward = { coins = 600, tickets = 0 },
        checkFn = function(snap) return (SaveSystem.GetStats().totalItemsWon or 0) - (snap.totalItemsWon or 0) end,
        target = 3,
    },
    {
        id    = 12,
        title = "在线时长",
        desc  = "累计在线 10 分钟",
        reward = { coins = 300, tickets = 0 },
        checkFn = function(snap) return math.floor((SaveSystem.GetPlayTime() - (snap.playTime or 0)) / 60) end,
        target = 10,
    },
}
