...
function AIPlayer.StartSealedBidThinking(gameState, infoSystem)
    ...
    for idx, player in ipairs(players) do
        if not player.isHuman then
            ...
            -- 用 InfoEstimation 模块计算估值（以区域期望值为先验锚点）
            local estimate, secureSv, expectedTotal, poolAvg, itemCount =
                _InfoEstimation.ComputeEstimate(idx, round, infoSystem, ai.infoStates, expectedValue, whTypeId)
...
function AIPlayer.DecideSealedBid(playerIdx, player, round)
    ...
    -- 获取估值和区域期望值
    local sv = ai.cachedEstimates[playerIdx]
    local expectedValue = _GameState.GetExpectedValue()
    local estimate = sv and sv.get() or expectedValue

    -- 第一层：意图判定（基于价值判断，不基于资金）
    local intent
    intent, ai.pumpActive = _Strategies.DecideIntent(playerIdx, player, round, estimate, ai.pumpActive, expectedValue)
    ai.lastIntents[playerIdx] = intent

    -- 第二层：策略计算（含倍率感知）
    local aiInfoState = ai.infoStates[playerIdx]
    local bidAmount = _Strategies.CalculateBid(intent, playerIdx, player, round, estimate, aiInfoState, expectedValue)
    ...
    -- 底价保护：COMPETE 意图的出价不应低于 expectedValue × 40%
    -- 最终安全网，防止策略折扣 + 随机波动把出价压到不合理的低位
    if intent == INTENT.COMPETE and expectedValue > 0 then
        local absoluteFloor = expectedValue * 0.40
