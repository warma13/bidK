...
-- 物品详情面板
local detailPanel = nil      -- UI widget（用于定位）
local detailInst = nil        -- ItemDetailPanel 实例（调用 Show/Hide 等）
...
-- 滑动选择状态
local isDragSelecting = false     -- 是否正在滑动选择
local dragSelectTouched = {}      -- 本次滑动已切换过的 item ref → true（防止反复切换）
local dragSelectPointerId = nil   -- 触摸ID（触屏用）
...
--- 根据屏幕坐标查找对应的物品（遍历可见的 imgPanel 做碰撞检测）
local function hitTestItemAtScreenPos(sx, sy)
    for i = 1, MAX_ITEM_IMAGES do
        local img = itemImages[i]
        if img and img:IsVisible() and imageToItem[i] then
...
--- 开始滑动选择
local function startDragSelect(item, touchId)
    isDragSelecting = true
    dragSelectTouched = {}
    dragSelectPointerId = touchId
    if item then
        dragSelectTouched[item] = true
        toggleItemSelection(item)
    end
end

--- 滑动选择中：检测当前指针下的物品
local function updateDragSelect(sx, sy)
    if not isDragSelecting then return end
    local item = hitTestItemAtScreenPos(sx, sy)
    if item and not dragSelectTouched[item] then
        dragSelectTouched[item] = true
        toggleItemSelection(item)
    end
end
...
local function exitSellMode()
    ...
    -- 隐藏物品详情
    if detailInst then detailInst:Hide() end
end
...
local function doSell()
    ...
    -- 通过统一入口加金币（自动乐观更新 + 失败回滚）
    MoneyManager.AddMoneyFromMenu(totalValue, "sell_items", {
        silent = true, -- SaveSystem.SaveNow() 会统一弹"已保存"，这里不重复提示
        ok = function()
            print("[Warehouse] Sell saved: +" .. totalValue)
        end,
        error = function(code, reason)
            -- 金币已被 AddMoneyFromMenu 自动回滚
            -- 但物品已移除，需要提示用户
...
function Panel.Show(onBack)
    ...
    for r = 1, totalRows do
        ...
        for c = 1, COLS do
            ...
            local slot = UI.Panel {
                ...
                alignItems = "center",
                onClick = function()
                    if not isSellMode then
                        Utils.PlayClick()
                        Panel._OnSlotClick(slotIdx)
                    end
                end,
                onPointerDown = function(event)
                    if not isSellMode then return end
                    if not gridMap then return end
                    local r2 = math.ceil(slotIdx / COLS)
                    local c2 = slotIdx - (r2 - 1) * COLS
                    local item = gridMap[r2] and gridMap[r2][c2]
                    if item then
                        Utils.PlayClick()
                        startDragSelect(item, event.pointerId)
                    end
                end,
            }
    ...
    for i = 1, MAX_ITEM_IMAGES do
        ...
        local imgPanel = UI.Panel {
            ...
            onClick = function()
                ...
                -- 出售模式的点击由 onPointerDown 处理（滑动选择）
            end,
            onPointerDown = function(event)
                if not isSellMode then return end
                Utils.PlayClick()
    ...
    local detailBackdrop = UI.Panel {
        position = "absolute",
        left = 0, top = 0, right = 0, bottom = 0,
        backgroundColor = { 0, 0, 0, 1 },  -- alpha=1: 肉眼不可见但拦截点击穿透
        visible = false,
        onClick = function(self, event)
            -- event.x/y 已被 UI 系统转换为基准像素（widget 本地坐标）
            -- backdrop 铺满全屏，本地坐标 = 屏幕基准像素坐标
            local lx, ly = event.x, event.y
            local item = hitTestItemAtScreenPos(lx, ly)
            if item then
                Panel._ShowItemInfo(item)
            else
                if detailInst then detailInst:Hide() end
    ...
    detailInst = ItemDetailPanel.New({
        position = "absolute",
        left = 0,
        top = 0,
        right = nil,
    ...
    detailPanel = detailInst:GetWidget()
    -- New() 初始化时可能触发 onShow，强制重置 backdrop 为隐藏
    detailBackdrop:SetVisible(false)
...
function Panel._OnSlotClick(slotIdx)
    ...
    if item then
        ...
        -- 点空格子隐藏详情
        if not isSellMode and detailInst then detailInst:Hide() end
    end
end

function Panel._ShowItemInfo(item)
    if not item or not detailInst then return end
    -- 用 realValue 或 baseValue 填充（ItemDetailPanel 读 realValue）
    if not item.realValue then
        item.realValue = item.baseValue or 0
    end
    if detailInst:IsVisible()
       and detailInst:GetCurrentItem() == item then
        detailInst:Hide()
        return
    end

    detailInst:Show(item)
...
function Panel.Update(dt)
    if not isDragSelecting or not isSellMode then
        if isDragSelecting then endDragSelect() end
        return
    end
    ...
    if stillDown then
        -- input 返回物理像素，hitTest 需要基准像素，除以 UI scale
        local s = UI.GetScale()
        updateDragSelect(sx / s, sy / s)
    else
        endDragSelect()
    end
end
