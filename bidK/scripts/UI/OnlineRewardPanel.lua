...
-- 云端同步
local CLOUD_SYNC_INTERVAL = 60 -- 每60秒同步一次在线时间到云端
...
local function SyncOnlineTimeToCloud()
    ...
    batch:Save("online_time_sync", {
        ok = function()
            print("[OnlineReward] Synced online time: " .. math.floor(onlineSeconds) .. "s")
        end,
        error = function(code, reason)
            print("[OnlineReward] Sync failed: " .. tostring(reason))
        end,
    })
end
        ...
        :Fetch({
            ok = function(values, iscores)
                ...
                if CheckDailyReset() then
                    -- 日期变了，已重置 —— 立即将重置后的状态写回云端
                    print("[OnlineReward] Daily reset")
                    local resetBatch = clientCloud:BatchSet()
                    resetBatch:SetInt(KEY_CLAIMED_BITS, 0)
                    resetBatch:SetInt(KEY_ONLINE_SECS, 0)
                    resetBatch:Set(KEY_ONLINE_DATE, onlineDate)
                    resetBatch:Save("online_daily_reset", {
                        ok = function()
                            print("[OnlineReward] Daily reset synced to cloud")
                        end,
                        error = function(code, reason)
                            print("[OnlineReward] Daily reset sync failed: " .. tostring(reason))
                        end,
                    })
                else
                    -- 恢复云端保存的在线时间
                ...
                print("[OnlineReward] Cloud loaded: claimed=" .. claimedBits
                    .. " secs=" .. math.floor(onlineSeconds))
            end,
            error = function(code, reason)
                cloudLoaded = true
                print("[OnlineReward] Cloud load failed: " .. tostring(reason))
            end,
        })
end
    ...
    MoneyManager.AddMoneyFromMenu(ms.coins, "在线奖励" .. ms.label, {
        ...
        ok = function()
            ...
            if ms.ticket then
                ...
                SaveSystem.AddTickets(ms.ticket, 1)
            end
            OnlineRewardPanel.RefreshAll()
...
--- 立即同步在线时间到云端（供外部调用，如游戏存档后）
function OnlineRewardPanel.SyncNow()
    cloudSyncTimer = 0
    SyncOnlineTimeToCloud()
end

--- 游戏退出时立即同步在线时间，避免最后一段时间丢失
function OnlineRewardPanel.Shutdown()
    if not clientCloud then return end
    if not cloudLoaded then return end
    if onlineSeconds <= 0 then return end

    local batch = clientCloud:BatchSet()
    batch:SetInt(KEY_CLAIMED_BITS, claimedBits)
    batch:SetInt(KEY_ONLINE_SECS, math.floor(onlineSeconds))
    batch:Set(KEY_ONLINE_DATE, onlineDate)
    batch:Save("online_time_final", {
        ok = function()
            print("[OnlineReward] Final sync OK: " .. math.floor(onlineSeconds) .. "s")
        end,
        error = function(code, reason)
            print("[OnlineReward] Final sync failed: " .. tostring(reason))
        end,
    })
end
