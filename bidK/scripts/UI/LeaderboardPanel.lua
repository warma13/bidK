    ...
    clientCloud:GetRankList("money_rank", start, PAGE_SIZE, {
        ok = function(rankList)
            ...
            for i, item in ipairs(rankList) do
                -- 过滤测试账号
                if not TEST_ACCOUNTS[item.userId] then
                    added = added + 1
                    ...
                    FillRow(r, globalIdx, tostring(item.userId),
                        displayMoney, isMe)
                end
            end
            ...
            -- 加载更多按钮状态
            if #rankList < PAGE_SIZE or loadedCount >= MAX_TOTAL then
                loadMoreBtn:SetVisible(false)
            else
