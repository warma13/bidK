...
local HOLD_DUR = 0.6
local MOVE_DUR = 0.3
local ANIM_TOTAL_TIMEOUT = 12.0   -- 动画总超时（秒）：防止动画管线卡死
local animTotalElapsed = 0
...
function CenterPanel.UpdateAnimation(dt)
    -- 总超时保护：防止动画管线卡死
    if animPhase ~= "none" or #UIState.infoAnimQueue > 0 then
        animTotalElapsed = animTotalElapsed + dt
        if animTotalElapsed >= ANIM_TOTAL_TIMEOUT then
            print("[CenterPanel] Animation total timeout (" .. ANIM_TOTAL_TIMEOUT .. "s), forcing reset")
