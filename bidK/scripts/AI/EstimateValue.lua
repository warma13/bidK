...
local function loadPool(warehouseTypeId)
    ...
    for _, modPath in ipairs(modules) do
        ...
        if ok and mod then
            -- 每个模块导出一个按稀有度分组的表
            for _, rarityId in ipairs({ "white", "green", "blue", "purple", "gold", "red" }) do
                local items = mod[rarityId]
                if items then
                    for _, item in ipairs(items) do
                        allItems[#allItems + 1] = {
                            rarity   = rarityId,
                            category = item.category or "unknown",
                            value    = (item.valueMin and item.valueMax)
                                and (item.valueMin + item.valueMax) / 2
                                or item.value or 0,
                        }
                    end
                end
    ...
    -- fallback: 如果池为空，使用 Config.ITEMS + Config.WHITE_ITEMS
    if #allItems == 0 then
        -- Config.WHITE_ITEMS
        if Config.WHITE_ITEMS then
            for _, item in ipairs(Config.WHITE_ITEMS) do
                allItems[#allItems + 1] = {
                    rarity   = "white",
                    category = item.category or "unknown",
                    value    = ((item.valueMin or 0) + (item.valueMax or 0)) / 2,
                }
            end
        end
        -- Config.ITEMS (有 baseValue)
        if Config.ITEMS then
            for _, item in ipairs(Config.ITEMS) do
                allItems[#allItems + 1] = {
                    rarity   = item.rarity or "white",
                    category = item.category or "unknown",
                    value    = item.baseValue or 0,
                }
            end
        end
    end
