...
-- 顾千鹤（信息驱动型）
strategyMap["info_driven"] = function(estimate, player, round, playerIdx, aiInfoState, expectedValue, analysis)
    local bid = baseCompeteBid(estimate, player, round, expectedValue, playerIdx, { analysis = analysis })
    ...
    if ch.activeSkill and ch.activeSkill.effect == "reveal_top3" then
        ...
        if skillUses > 0 and round >= 2 then
            ...
            if aiInfoState and topItems then
                ...
                -- 用新估值重新计算出价
                bid = baseCompeteBid(newEstimate, player, round, expectedValue, playerIdx, { analysis = analysis })
                -- 信息充分 → 可以更激进
...
-- 沈惊鸿（博弈型）
strategyMap["gambler"] = function(estimate, player, round, playerIdx, aiInfoState, expectedValue, analysis)
    local bid = baseCompeteBid(estimate, player, round, expectedValue, playerIdx, { analysis = analysis })
...
-- 苏巧巧（套利型）
strategyMap["arbitrage"] = function(estimate, player, round, playerIdx, aiInfoState, expectedValue, analysis)
    local bid = baseCompeteBid(estimate, player, round, expectedValue, playerIdx, { analysis = analysis })
...
strategyMap["banker"] = function(estimate, player, round, playerIdx, aiInfoState, expectedValue, analysis)
...
strategyMap["grower"] = function(estimate, player, round, playerIdx, aiInfoState, expectedValue, analysis)
...
strategyMap["veteran"] = function(estimate, player, round, playerIdx, aiInfoState, expectedValue, analysis)
    local bid = baseCompeteBid(estimate, player, round, expectedValue, playerIdx, { analysis = analysis })
...
-- 沈玉珂/吴鉴之（精准狙击型）
strategyMap["sniper"] = function(estimate, player, round, playerIdx, aiInfoState, expectedValue, analysis)
    local bid = baseCompeteBid(estimate, player, round, expectedValue, playerIdx, { analysis = analysis })
...
strategyMap["specialist"] = function(estimate, player, round, playerIdx, aiInfoState, expectedValue, analysis)
    local bid
    local bonus = _GameState.GetValueBonus(playerIdx)
    if bonus > 0 then
        -- 有专精加成：底价标准，但出价更激进
...
function Strategies.CalculateBid(intent, playerIdx, player, round, estimate, aiInfoState, expectedValue, analysis)
    ...
    -- 正常竞争：策略计算（将 analysis 通过 opts 传递给 baseCompeteBid）
    local strategyFn = strategyMap[p.style]
    local baseBid
    if strategyFn then
        baseBid = strategyFn(estimate, player, round, playerIdx, aiInfoState, expectedValue, analysis)
    else
        baseBid = baseCompeteBid(estimate, player, round, expectedValue, playerIdx, { analysis = analysis })
    end
